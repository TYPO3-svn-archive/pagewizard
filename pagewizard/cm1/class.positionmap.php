...
class positionMap {
	...
	function positionTree($id,$pageinfo,$perms_clause,$R_URI)	{
		...
		foreach($t3lib_pageTree->tree as $cc => $dat)	{
				...
				if ($prev_dat['row']['uid']==$id)	{
					if (!$this->dontPrintPageInsertIcons && $this->checkNewPageInPid($id) && !($prev_dat['invertedDepth']>$t3lib_pageTree->tree[$cc]['invertedDepth']))	{	// 1) It must be allowed to create a new page and 2) If there are subpages there is no need to render a subpage icon here - it'll be done over the subpages...
						...
						$code.='<span class="nobr">'.
							...
							'<a href="#" onclick="window.location.href=\'index.php?id='.$id.'&cmd=crPage&positionPid='.$id;
						if($this->fe_mode) {
							$code .= '&pagewizard_fe_mode=1&pos_pid='.$id;
						}
						$code .= 
							'\';return false;" onmouseover="'.htmlspecialchars('changeImg(\'mImgSubpage'.$cc.'\',0);').'" onmouseout="'.htmlspecialchars('changeImg(\'mImgSubpage'.$cc.'\',1);').'">'.
				...
				if ($prev_dat['invertedDepth']>$t3lib_pageTree->tree[$cc]['invertedDepth'])	{	// If going down
					...
					if (!$this->dontPrintPageInsertIcons && $this->checkNewPageInPid($prev_dat['row']['pid']))	{
						...
						$code.='<span class="nobr">'.
							...
							'<a href="#" onclick="window.location.href=\'index.php?id='.$id.'&cmd=crPage&positionPid='.$prevPid;
						if($this->fe_mode) {
							$code .= '&pagewizard_fe_mode=1&pos_pid='.$prevPid;
						}
						$code .= '\';return false;" onmouseover="'.htmlspecialchars('changeImg(\'mImgAfter'.$cc.'\',0);').'" onmouseout="'.htmlspecialchars('changeImg(\'mImgAfter'.$cc.'\',1);').'">'.
			...
			if (!$this->dontPrintPageInsertIcons && $this->checkNewPageInPid($dat['row']['pid']))	{
				...
				$code .= '\';return false;" onmouseover="'.htmlspecialchars('changeImg(\'mImg'.$cc.'\',0);').'" onmouseout="'.htmlspecialchars('changeImg(\'mImg'.$cc.'\',1);').'">'.
			...
			$t_code='<span class="nobr"><table border="0" cellspacing="0" cellpadding="0"><tr><td>'.
				$dat['HTML'].'</td><td valign="middle">'.
		...
		if ($prev_dat['row']['uid']==$id)	{
			if (!$this->dontPrintPageInsertIcons && $this->checkNewPageInPid($id))	{
				...
				$code .= '\';return false;" onmouseover="'.htmlspecialchars('changeImg(\'mImgSubpage'.$cc.'\',0);').'" onmouseout="'.htmlspecialchars('changeImg(\'mImgSubpage'.$cc.'\',1);').'">'.
					'<img'.t3lib_iconWorks::skinImg($this->backPath,'gfx/newrecord_marker_d.gif','width="281" height="8"').' name="mImgSubpage'.$cc.'" border="0" align="top" title="'.$this->insertlabel().'" alt="" />'.
					'</a></span><br />';
			}
		}
		...
		for ($a=$latestInvDepth;$a<=$this->depth;$a++)	{
			...
			if (!$this->dontPrintPageInsertIcons && $this->checkNewPageInPid($dat['row']['pid']))	{
				...
				$code .= '\';return false;" onmouseover="'.htmlspecialchars('changeImg(\'mImgEnd'.$a.'\',0);').'" onmouseout="'.htmlspecialchars('changeImg(\'mImgEnd'.$a.'\',1);').'">'.
	...
	function onClickEvent($pid,$newPagePID)	{
		...
		if ($TSconfigProp['overrideWithExtension'])	{
			...
			if (t3lib_extMgm::isLoaded($TSconfigProp['overrideWithExtension']))	{
				//print "override";
				$onclick = "window.location.href='".t3lib_extMgm::extRelPath($TSconfigProp['overrideWithExtension']).'cm1/index.php?cmd=crPage&positionPid='.$pid;
				if($this->fe_mode) {
					$onclick .= '&pagewizard_fe_mode=1&pos_pid='.$pid;
				}
				$onclick .= "';";
				#print $onclick;
				return $onclick;
	...
	function linkPageTitle($str,$rec)	{
		global $LANG;
		if($str == "") {
			return "<i>".$LANG->getLL('no_title')."</i>";
		}
		return $str;
	}
	...
	function getModConfig($pid)	{
		...
		#print $this->modConfigStr;
		#print_r($this->getModConfigCache[$pid]['properties']);
